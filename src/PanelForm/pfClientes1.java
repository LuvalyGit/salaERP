 ...
 * @author DavidAlcaman
 */
 public class pfClientes1 extends javax.swing.JPanel {
                ...
                .addGroup(jPanel1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    .addGroup(jPanel1Layout.createSequentialGroup()
                        ...
                        .addGroup(jPanel1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING, false)
                            ...
                            .addComponent(txCiudad, 0, javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
    ...
    private void cbCodigoOcActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cbCodigoOcActionPerformed
        ...
        try{
          if(txRut.getText().isEmpty()) return;
           ...
           Rs = Sql.Select("select giro,direccion,ciudad,comuna,region, correo\n" +
                            "from clicontacto\n" +
            ...
            if(Rs.getRow()>0){
                ...
                int ciudades = txCiudad.getItemCount();
                int comunas = txComuna.getItemCount();
                for(int t = 0; t < ciudades; t ++ ){
                    if(txCiudad.getItemAt(t).toString().equals(Rs.getString("ciudad").trim())){
                        txCiudad.setSelectedIndex((t));
                        break;
                    }   
                }
                for(int u = 0; u < comunas; u ++ ){
                    if(txComuna.getItemAt(u).toString().equals(Rs.getString("comuna").trim())){
                        txComuna.setSelectedIndex((u));
                        break;
                    }   
                }
    //            try {txGiro.setText(Rs.getString("Giro").trim());} catch (Exception e) {txGiro.setText("");}
    //            try {txDireccion.setText(Rs.getString("Direccion").trim());} catch (Exception e) {txDireccion.setText("");}
    //            try {txCiudad.setSelectedItem(Rs.getString("Ciudad").trim());} catch (Exception e) {txCiudad.setSelectedItem("");}
    //            try {txComuna.setSelectedItem(Rs.getString("Comuna").trim());} catch (Exception e) {txComuna.setSelectedItem("");}
...
//             Sql.ExeSql("DELETE FROM clicontactopersonas WHERE rut=" + txRut.getText().trim() + 
