        ...
        while(dfTm.getRowCount()>0)
         ...
         try{
            ...
            Query = "select nrodocto from ctacteprv \n" +
                    "where tipdocto IN ('NPP') and estado = 2 ";
        ...
        String strUbicacion = "";
         ...
         try{ 
                ...
                if (nombrePalabra.substring(0,3).trim().equals("LVL") || nombrePalabra.substring(0,4).trim().equals("TRAN") || 
                    ...
                    nombrePalabra.substring(0,3).trim().equals("BOD")    
                 ){
...
//                Sql.ExeSql(qryUpd);
    ...
    private void txtSkuKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txtSkuKeyPressed
        ...
        if(evt.getKeyCode() == KeyEvent.VK_ENTER){
            ...
            if (nombrePalabra.trim().contains("TRAN.1004.1") ){    //Si es NCC
           ...
           if (nombrePalabra.substring(0,3).trim().equals("LVL") || nombrePalabra.substring(0,4).trim().equals("TRAN") || 
               ...
               trae_ubicacion(nombrePalabra.trim());
               txtSku.setText("");
               ...
               txtSku.requestFocus();
                ...
                try {
                   ...
                   Rs = Sql.Select("select cpt.sku, cpt.nrodocto FROM ctacteprvdet cpt\n" +
                                   ...
                                   "WHERE cpt.sku = '"+txtSku.getText().trim()+"' AND cpt.tipdocto IN ('NPP') AND cp.estado = 2 AND cpt.recibido > 0");
                    ...
                    if (Nota == 0){
                        ...
                        for (int i = 0; i < Grilla_prod.getRowCount(); i++) {
                            ...
                            if (Grilla_prod.getValueAt(i, 0).equals(txtSku.getText().trim())) {
                                ...
                                boolean selecciona = (boolean) Grilla_prod.getValueAt(i,6);
                                
                                if (selecciona){
             
                                    Grilla_prod.setValueAt(false, i, 6);
                                    Grilla_prod.changeSelection(i, 0, false, false);  //283591672
                                    
                                    contselec--;
             
                                }else if (!selecciona) {
                                    
                                    Grilla_prod.setValueAt(true, i, 6);
                                    Grilla_prod.changeSelection(i, 0, false, false);
                   ...
                   Logger.getLogger(AsignaUbicacion_Producto.class.getName()).log(Level.SEVERE, null, ex);
               }
   ...
   // TODO add your handling code here:
    ...
    private void Grilla_prodMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_Grilla_prodMouseClicked
        ...
        int nFilasSelec = Grilla_prod.getSelectedRow(); 
                
        //int fila[] = Grilla_prod.getSelectedRows();
        DefaultTableModel modelotabla = (DefaultTableModel) Grilla_prod.getModel();

       // int row = fila[5];
        
        int row = nFilasSelec;
        int f = Grilla_prod.convertRowIndexToModel(row);
        
        String Sku = (String)modelotabla.getValueAt(f, 0);
        String nombre = (String) modelotabla.getValueAt(f, 1);
        //String cant = (String)modelotabla.getValueAt(f, 2);
        String cant = String.valueOf(modelotabla.getValueAt(f, 2));
                
        System.out.println("LA FILA ES : "+Sku+" - "+nombre+" - "+cant); 
...
//        }
    ...
    private void btUbicarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btUbicarActionPerformed
        ...
        Carga_Prod_Ubicacion_NPP(txtUbicacion.getText().trim());
            ...
            Nota = 1;
            System.out.println("LAS FILAS SON : "+Grilla_prod.getRowCount());
    ...
    private void btCerrarNotaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btCerrarNotaActionPerformed
        ...
        try {
            ...
            if (Grilla_prod.getRowCount() > 0){
            
            
                fmMain.Mensaje("Esta Orden aún tiene Productos para Ubicar !!");
                return;
        ...
        try{
            ...
            if (Sql3.GetRowCount() > 0){
                ...
                while (Rs3.next()) {
                    ...
                    Rs = Sql.Select("SELECT sku, pventa_web2 FROM producto \n" + //76.295.788-4
                                    "WHERE sku = '"+SkuDet.trim()+"'" );
                    ...
                    if (Sql.GetRowCount() > 0){
                        ...
                        valor_web = Rs.getDouble("pventa_web2");
...
//                enter_ubicacion();
            ...
            System.out.println(e.getMessage());
            Logger.getLogger(AsignaUbicacion_Producto.class.getName()).log(Level.SEVERE, null, e);
            Sql2.Rollback();
...
//                    "WHERE ubicacion = '" + ubicacion  + "' and mp.cant <>0 and mp.sku is not null and ct.nrodocto = "+Grilla_Notas.getValueAt(Grilla_Notas.getSelectedRow(), 0).toString().trim()+" and ct.tipdocto IN ('NPP') " +
//                    "order by case when mp.fecha_mod is null then EXTRACT(day from current_date-mp.fecha) else EXTRACT(day from current_date-mp.fecha_mod) END DESC"; 
//            
             ...
             Query=" select mp.sku Sku, ct.recibido as cant, ct.ubicado, \n" +
                    ...
                    "FROM mt_productos mp \n" +
                    "LEFT JOIN ctacteprvdet ct ON mp.sku=ct.sku\n "+
                    "WHERE ubicacion = '" + ubicacion  + "' and (ct.recibido-ct.ubicado) > 0 and mp.sku is not null and ct.nrodocto = "+Grilla_Notas.getValueAt(Grilla_Notas.getSelectedRow(), 0).toString().trim()+" and ct.tipdocto IN ('NPP') " +
                    "order by case when mp.fecha_mod is null then EXTRACT(day from current_date-mp.fecha) else EXTRACT(day from current_date-mp.fecha_mod) END DESC"; 
            ...
            while(Rs.next()){
                ...
                if(producto.getString("cchile") != null){                     //Si está en codchile
                     ...
                     if (!producto.getString("Sku").equals(SkuAnt)){        //Que no se repita el SKU
                         ...
                         dfTm.addRow(new Object[]{Rs.getString("Sku"),producto.getString("Nombre"),Rs.getDouble("cant")-Rs.getDouble("ubicado"),"0",Rs.getString("fechita"),Rs.getInt("dias"),false});
                         registros++;
                         }
