 ...
 */
public class jdContactosCli extends javax.swing.JDialog {
    String Eliminados;
    int EliminadosCont;
    String IdContacto="";
     ...
     */
    public jdContactosCli(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
    }
    ...
    private void initComponents() {
        ...
        GrillaContactos.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "NOMBRE", "FONO", "CELULAR", "EMAIL", "CARGO", "Id"
            }
        ) {
        ...
        GrillaContactos.addMouseListener(new java.awt.event.MouseAdapter() {
            public void mouseClicked(java.awt.event.MouseEvent evt) {
                GrillaContactosMouseClicked(evt);
            }
        });
        GrillaContactos.addKeyListener(new java.awt.event.KeyAdapter() {
            public void keyReleased(java.awt.event.KeyEvent evt) {
                GrillaContactosKeyReleased(evt);
            }
            public void keyPressed(java.awt.event.KeyEvent evt) {
                GrillaContactosKeyPressed(evt);
            }
        });
    ...
    private void CargaContactosPrv(){
        ...
        try {
                ...
                Rs = Sql.Select("select p.id,p.nombre,p.fono1,p.fono2,p.email,g.nombre as cargo\n" +
                           ...
                           "where p.rut=" + RutMaster + " order by p.nombre");
                while(Rs.next()){
                    try {
...
//                           "where p.rut=" + RutMaster);
                ...
                Rs = Sql.Select("select cli.nombre,cli.fono,cli.celular,cli.email,cli.id, par.nombre as CARGO \n" +
                     "from clicontactopersonas cli\n" +
                     "left join par_general par on cli.id_cargo=par.codigo \n" +
                     "where par.tipo='" + Tipo + "' and cli.rut="+ RutMaster + "\n"+
                     "and cli.codigo_oc="+ StrCodigo_OC);
                ...
                while(Rs.next()){
                    ...
                    try {
                        Fono1=Rs.getString("fono").trim();
                    } catch (Exception e) {
                        Fono1="";
                    }
                    try {
                        Fono2=Rs.getString("Celular").trim();
                    } catch (Exception e) {
                        Fono2="";
                    }
                    try {
    ...
    public String Show(String Rut,String Codigo_OC,  String Tipo, boolean Seleccionar){
        
        RutMaster = Rut;
        StrCodigo_OC= Codigo_OC;
...
//        }
    ...
    private void btAgregarContactoActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btAgregarContactoActionPerformed
        ExeSql Sql = new ExeSql();
        ResultSet Rs;
        String Tipo = "TIPCONTACTOCLI";
        int i = GrillaContactos.getRowCount();
        
        DefaultTableModel TModel = (DefaultTableModel) GrillaContactos.getModel();

        jdAddDatosPersonaCli Persona = new jdAddDatosPersonaCli(null, true);
        Persona.ConTipo("");
        Persona.Nuevo(1, Integer.valueOf(RutMaster), Integer.valueOf(StrCodigo_OC));
...
//                        "(rut,cdodigo_oc, nombre,fono,celular,email) values (" + 
//                        RutMaster + "," + StrCodigo_OC +  ", '" +
...
//            Sql.Commit();
    ...
    private void btEliminarContactoActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btEliminarContactoActionPerformed
       ExeSql Sql = new ExeSql();
        ...
        try {
            Sql.ExeSql("DELETE FROM clicontactopersonas WHERE id = " + GrillaContactos.getModel().getValueAt(GrillaContactos.getSelectedRow(),5).toString());
            Sql.Commit();
            CargaContactos("TIPCONTACTOCLI");
        } catch (Exception e) {
            System.out.println(e.getMessage());
        } finally{
            Sql.Close();
    ...
    private void GrillaContactosMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_GrillaContactosMouseClicked
         if(evt.getClickCount()==2){
         btAceptar.doClick();
         }
    }//GEN-LAST:event_GrillaContactosMouseClicked

    private void GrillaContactosKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_GrillaContactosKeyReleased

      

    }//GEN-LAST:event_GrillaContactosKeyReleased

    private void GrillaContactosKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_GrillaContactosKeyPressed
          if(evt.getKeyCode() == evt.VK_ENTER) {
            btAceptar.doClick();
            }
        else{
        char letra = evt.getKeyChar();
        int x = GrillaContactos.getSelectedRow();
        int encontrados = 0;

        for (int y = x + 1; y < GrillaContactos.getRowCount(); y++) {
            if (GrillaContactos.getValueAt(y, 0).toString().toUpperCase().substring(0, 1).contains(String.valueOf(letra).toUpperCase())) {
                GrillaContactos.changeSelection(y, 0, false, false);
                GrillaContactos.requestFocus();
                encontrados++;
                break;
            }
        }
        if (encontrados==0)
        {
        for (int z = 0; z < GrillaContactos.getRowCount(); z++) {
            if (GrillaContactos.getValueAt(z, 0).toString().toUpperCase().substring(0, 1).contains(String.valueOf(letra).toUpperCase())) {
                GrillaContactos.changeSelection(z, 0, false, false);
                GrillaContactos.requestFocus();
                break;
    ...
    }//GEN-LAST:event_GrillaContactosKeyPressed
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(jdContactosCli.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                jdContactosCli dialog = new jdContactosCli(new javax.swing.JFrame(), true);
